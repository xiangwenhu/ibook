 ...
 */
'use strict';
var DEFAULT_URL = 'rfc7231.txt.pdf';
    ...
    (function (root, factory) {
      ...
      var PDFViewerApplication = {
        ...
        load: function pdfViewLoad(pdfDocument, scale) {
          ...
          if(!isFromFS){
            var filer = new Filer();
            filer.init({persistent: false, size: 1024 * 1024}, (fs) => {
            // filer.size == Filer.DEFAULT_FS_SIZE
              //filer.isOpen == true
              //filer.fs == fs
              let fileName = this.baseUrl.split('?')[0].split('/').pop();
              this.pdfDocument.getData().then(function getDataSuccess(data) {
                var blob = pdfjsLib.createBlob(data, 'application/pdf')

                filer.write(fileName, {data: blob, type: 'application/pdf'}, function(fileEntry, fileWriter) {
      ...
      var HOSTED_VIEWER_ORIGINS = [
        ...
        'https://ibook.file.core.windows.net',
        'https://localhost:8082',
        'https://172.16.8.108:8082',
        'https://babydairy2017.cloudapp.net:8082',
        'http://localhost:8080'
      ];
      ...
      webViewerOpenFileViaURL = function webViewerOpenFileViaURL(file) {
        ...
        if (file) {
          //
          let fileName = file.split('?')[0].split('/').pop();
          filer.init({},(fs)=>{
            filer.ls('./',files =>  {
              var fileInfo = files.find(f=>f.name == fileName);
              if(fileInfo){
                filer.open(fileName,file=>{          
                  var reader = new FileReader();
                  reader.onload = e => {
                    isFromFS = true;
                    PDFViewerApplication.open(reader.result)
                  };
